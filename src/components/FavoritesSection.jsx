  ...
  return (
    <div className="card tint" style={{ marginTop: 14 }}>
      <div className="mini-head">
        <span className="star">⭐️</span> 收藏夹
        <button className="btn ghost sm" disabled={syncing} onClick={sync}
          style={{ marginLeft: 'auto' }}>{syncing ? '同步中' : '同步B站'}</button>
      </div>
      <div className="sub" style={{ marginTop: -4 }}>已收录 {items.length} 条</div>

      <div className="cat-bar" style={{ margin: '8px 0' }}>
        {['全部', ...CATS].map(c => (
